...
def make_datasets(power_dataset: str,
    ...
    """
    Prepare data for use in statsmodels statespace model.
    :param power_dataset: [gym | johnson | johnson_gym (both) | no_weather]
    :param lags: [day|week|month|season|year] (list)
    :param resample: [True | False] resample to daily frequency.
    :param scaler: [minmax: scaling between (0,1) | standard: standardize to mean 0 and std 1]
    :return: Data object
    """
    ...
    if 'johnson_gym' in power_dataset:
        ...
        stationary = raw_data_johnson_gym.transform(lag=lags,
                                                    resample=resample,
                                                    scaler=scaler)
        ...
        raw_data_gym = Data.get_data(datafile='data/4Y_Historical.csv',
                                     powerfile='data/gym_from_2010_04_06_to_2020_12_31.csv')
        stationary = raw_data_gym.transform(lag=lags,
                                            resample=resample,
                                            scaler=scaler)
        ...
        raw_data_johnson = Data.get_data(datafile='data/4Y_Historical.csv',
                                         powerfile='data/maabarot_johnson_from_2010_04_22_to_2020_12_31.csv')
        stationary = raw_data_johnson.transform(lag=lags,
                                                resample=resample,
                                                scaler=scaler)
        stationary.df['max_power'] = stationary.df['max_power'].astype(np.float64)

    elif 'no_weather' in power_dataset:
        raw_data_johnson_gym = Data.get_data(datafile='data/4Y_Historical.csv',
                                             powerfile='data/maabarot_johnson_from_2010_04_22_to_2020_12_31.csv')
        raw_data_gym = Data.get_data(datafile='data/4Y_Historical.csv',
                                     powerfile='data/gym_from_2010_04_06_to_2020_12_31.csv')
        raw_data_johnson_gym.df = pd.concat([raw_data_johnson_gym.df['max_power'], raw_data_gym.df['max_power']], axis=1,
                                            join='inner')
        raw_data_johnson_gym.df.columns = ['max_power_johnson', 'max_power_gym']
